...
<div class="input-group">
  <input name="acode" type="text" class="form-control" placeholder="donnez un code promo pour avoir une remise" aria-label="Search" aria-describedby="basic-addon2"/>
...
{ 
 ...
 //<input type="hidden" name="promotion" class="form-control" id="promotion"  value="$pri" > 


    if (mysqli_num_rows($result_tasks) == 0) {
  echo  "  ";
   }
else {//echo  "code trouvee  ";
  // $a=$_GET['id'];
//echo  "$a  ";
$query2 = "SELECT * FROM voiture where id = '$_GET[id]' ";
   $result_tasks1 = mysqli_query($conn, $query2); 
   ($row1 = mysqli_fetch_array($result_tasks1)) ;
   $pri1 = $row1['prix']; 
   $prixf=($pri1* (1-($pri/100)));
$query1 =" UPDATE `voiture` SET `prix` = $prixf WHERE `voiture`.`id` = '$_GET[id]'";
  mysqli_query($conn, $query1);
    $query3= "DELETE FROM promotion WHERE codepromo = $acode";
